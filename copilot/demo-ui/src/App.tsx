...
export default function App() {
  ...
  const handleSend = async (prompt: string) => {
    ...
    try {
      ...
      // VITE_API_BASE is '/copilot/api' in production (injected at build time),
      // empty string in local dev (Vite proxy handles /api → agent:8000).
      const apiBase = import.meta.env.VITE_API_BASE ?? '/api'
      const res = await fetch(`${apiBase}/chat`, { method: 'POST', headers, body: JSON.stringify(body) })
